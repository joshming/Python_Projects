...
class Gym:
    ...
    def add_instructor(self, instructor: Instructor) -> bool:
        ...
        True
        """
        instructor_num = instructor.get_id()
        if instructor_num not in self._instructors:
            self._instructors[instructor_num] = instructor
            return True
        return False
    ...
    def schedule_workout_class(self, time_point: datetime, room_name: str,
        ...
        teachable = instructor.can_teach(workout)

        if time_point not in self._schedule and teachable:
    ...
    def register(self, time_point: datetime, client: str, workout_name: str) \
        ...
        for room in rooms:
            if workout in self._schedule[time_point][room] and can_register:
    ...
    def offerings_at(self, time_point: datetime) -> List[Tuple[str, str, str]]:
        ...
        offerings = []
        for room in self._schedule[time_point]:
            offer = (self._schedule[time_point][room][0].name,
                     self._schedule[time_point][room][1].get_name(), room)
        ...
        for time in self._schedule:
            if time1 <= time <= time2:
                for room in self._schedule[time]:
                    id_num = self._schedule[time][room][0].get_id()
                    hours[id_num] += 1
        return hours
    ...
    def payroll(self, time1: datetime, time2: datetime, base_rate: float) \
        ...
        payed = []
        for id_num in ids:
            instructor = self._instructors[id_num]
            payed.append((id_num, instructor.name, hours[id_num],
                          hours[id_num] * base_rate +
                          instructor.get_num_certificates() * BONUS_RATE *
                          hours[id_num]))
